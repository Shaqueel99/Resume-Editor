...
A Streamlit app that compares your résumé against a job description, scores how well it matches, and rewrites weak spots to surface the job description's own terminology — plus a second tool that drafts a tailored cover letter from the same inputs. It's for job seekers who want to tailor a résumé and cover letter to a specific posting without rewriting everything by hand.

## Problem Statement

Tailoring a résumé and cover letter to every job posting is tedious, and it's easy to miss required keywords that ATS (applicant tracking system) filters and recruiters scan for. This app automates that comparison: it tells you exactly which required skills your résumé is missing, suggests wording changes anchored to your real bullet points (never invented experience), and drafts a cover letter grounded in the same résumé-to-job-description analysis — saving the manual re-reading and re-writing normally needed for each application.
...
3. Create a `.env` file in the project root and set which model to use. For Google Gemini:
   ...
   - Local Ollama (no API key, more consistent, requires `ollama serve` running): `MODEL=ollama/gemma2:2b`, `OLLAMA_API_BASE=http://localhost:11434`
